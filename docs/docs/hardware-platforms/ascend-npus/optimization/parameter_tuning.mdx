...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>`--cp-strategy`</td>
      <td><strong>(DeepSeek V3.2 DSA-specific)</strong> Controls how the long sequence is split across context parallel ranks: `zigzag` (legacy name `in-seq-split`) divides each sequence uniformly across CP ranks, optimal for single-request prefill. `interleave` (legacy name `round-robin-split`; code default) distributes tokens by index mod CP size, supporting multi-batch prefill. DeepSeek V3.2 requires `interleave`. Only effective when `--enable-prefill-cp` is enabled.</td>
      <td>`interleave`</td>
    </tr>
    <tr>
      <td>`--attn-cp-size`</td>
