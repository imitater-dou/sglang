...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>`--cp-strategy`</td>
      <td><strong>(DeepSeek V3.2 DSA-specific)</strong> Controls how the long sequence is split across context parallel ranks: `zigzag` assigns each rank one early and one late sequence block (legacy `in-seq-split`), optimal for single-request prefill. `interleave` (code default; legacy `round-robin-split`) assigns token indices modulo the CP size, supporting multi-batch prefill. Only effective when `--enable-prefill-cp` is enabled.</td>
      <td>`interleave`</td>
    </tr>
    <tr>
      <td>`--attn-cp-size`</td>
